...
const Task = ({description, id, isDone}) => {
  ...
  return (
    <div className="Task">
      {!open_edit ? (
        <textarea
          className={isDone ? "done desc" : "undone desc"} style={{backgroundColor:'transparent',fontSize:'60px',border:'none',height:'80px',color:'white',marginTop:'15px'}}
          rows="10"
          cols="33"
          value={description}
          readOnly={true}
      ...
      <button onClick={() => dispatch(deleteTask(id))}>Effacer</button>
      <button onClick={() => dispatch(doneTask(id))}>
        {isDone ? "Fais" : "Pas Fais"}
      </button>
    </div>
  );
};
